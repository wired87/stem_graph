...
from firegraph._db.config import duck_db_path, duck_db_verbose
from firegraph._db.log_facade import db_log
from firegraph._db.queries import duck_row_from_id, get_column
...
class DBManager:
    ...
    def perform_vs(
        self,
        table,
        embedding_comparishon
    ):
        print("perform_vs...")
        n = .8

        goterm_match_ids = set()
        ...
        print("ids[0]", ids[0], type(ids[0]))

        # perform vs
        # Convert lists to NumPy arrays for hyper-fast vectorized matrix math
        Q = np.array(embedding_comparishon, dtype=np.float32)
        D = np.array(embeddings, dtype=np.float32)
        ...
        for i, scores in enumerate(similarity_matrix):
            for j, score in enumerate(scores):
                if score > n:
                    goterm_match_ids.add(ids[j])

        print("matched ids identified:", len(goterm_match_ids), goterm_match_ids)
        return list(goterm_match_ids)
